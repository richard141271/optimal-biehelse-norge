...
type ResponsePayload =
  | { ok: true; ticket: { id: string; revealed: boolean; isWinner?: boolean; prizeName?: string | null; claimed?: boolean } }
...
export async function GET(_request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  if (error) {
    return NextResponse.json<ResponsePayload>({ ok: false, feil: "Kunne ikke hente skrapelodd." }, { status: 400 })
  }
  if (!data) {
