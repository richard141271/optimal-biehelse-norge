...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    payload = (await request.json()) as UploadTokenPayload
  } catch {
    return NextResponse.json({ ok: false, feil: "Ugyldig forespørsel." }, { status: 400 })
  }

  const name = String(payload.name ?? "vedlegg").trim() || "vedlegg"
  const type = String(payload.type ?? "").trim()
  const size = Number(payload.size ?? 0)
  if (!Number.isFinite(size) || size <= 0) {
    return NextResponse.json({ ok: false, feil: "Ugyldig fil." }, { status: 400 })
  }
  if (size > MAX_VEDLEGG_BYTES) {
    return NextResponse.json(
      { ok: false, feil: `${name} er for stor. Hver fil kan maks være 15 MB.` },
      { status: 400 }
    )
  }
  ...
  if (error || !data?.token) {
    ...
    return NextResponse.json(
      { ok: false, feil: msg ? `Kunne ikke starte opplasting: ${msg}` : "Kunne ikke starte opplasting." },
      { status: 400 }
    )
  }
